...
public class AutoBalance extends Action {
    ...
    public void init() {
        dampeningFactor = 1;
        lastPitchSign = MathUtils.getSign(Subsystems.gyro.getPitchDegrees());
        balancedTickCount = 0;
    }
    ...
    public void periodic() {
        double pitch = Subsystems.gyro.getPitchDegrees(); // Get the NavX pitch
        ...
        Subsystems.driveBase.drive(pitch / 9 * (SPEED / (dampeningFactor)), 0, 0); // Move according to the pitch
        lastPitchSign = MathUtils.getSign(Subsystems.gyro.getPitchDegrees());
    }
